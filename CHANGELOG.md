...
## 2026-06-13 — Сканирование статуса при запуске и пропуск установленного ПО

Переработан поток `install-software.ps1`: теперь при каждом запуске скрипт
один раз сканирует систему и классифицирует все программы из
`windows-software.json` на три группы. Уже установленное ПО (как через
Chocolatey, так и вне его) больше не переустанавливается через Chocolatey.

### Изменения в `install-software.ps1`

- Новая функция `Update-InstallStatus` — за один проход получает
  `Get-ChocoInstalledIds` и `Get-RegistryInstalledPrograms` и делит пакеты на:
  - **установлено через Chocolatey** (`choco_id` есть в `choco list`);
  - **установлено вне Chocolatey** (найдено в реестре, но не в `choco list`);
  - **не установлено** (нет ни там, ни там).
  Результат кэшируется в `$script:InstallStatus`.
- Главное меню теперь показывает счетчики по трем группам и вызывает
  сканирование один раз при старте (раньше реестр сканировался повторно
  на каждый пакет).
- Новый пункт **«1. Установить неустановленные пакеты (через Chocolatey)»**
  (`Install-NotInstalled`) — ставит через Chocolatey только пакеты из группы
  «не установлено». Пакеты без `choco_id` пропускаются с подсказкой
  `official_url`. После установки выполняется пересканирование.
- Новый пункт **«2. Подробнее (списки по статусу)»** (`Show-StatusDetails`) —
  три списка по категориям: через Chocolatey `[+]`, вне Chocolatey `[~]`,
  не установлено `[-]`.
- Новый пункт **«3. Обновить данные (пересканировать систему)»**.
- Удалены прежние пункты автоматической/интерактивной установки и список
  установленных программ — их заменяют новые пункты выше.
- Пункт **«4. Проверить ПО вне Chocolatey (отчет .txt)»** сохранен без
  изменений.

### Технические замечания

- `install-software.ps1` сохранен в UTF-16 LE с BOM (для корректной кириллицы
  в PowerShell 5.1). Синтаксис проверен парсером PowerShell (0 ошибок).
- Проверено на реальной машине: из 17 программ списка корректно определены
  1 через Chocolatey, 5 вне Chocolatey, 11 не установлено.
