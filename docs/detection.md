...
- **`install-software.ps1`**, пункт меню **4** (`Show-NonChocoSoftware`):
  выполняет проверку, печатает результат и пишет отчет. Кроме того, при
  установке (`Install-Software`) перед установкой через Chocolatey
  проверяется реестр — если программа уже стоит вне Chocolatey, установка
  пропускается.
